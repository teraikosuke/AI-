...
function buildMsWhere({ scope, departmentKey, metricKey, periodId, advisorUserId }) {
  ...
  if (Number.isFinite(advisorUserId) && advisorUserId > 0) {
    ...
    clause += ` AND advisor_user_id=$${params.length}`;
  } else {
    clause += " AND advisor_user_id IS NULL";
  }
  return { clause, params };
}

async function getMsTargets(client, { scope, departmentKey, metricKey, periodId, advisorUserId }) {
  const where = buildMsWhere({ scope, departmentKey, metricKey, periodId, advisorUserId });
  const periodRes = await client.query(
    `SELECT target_total
     FROM ms_period_targets
     WHERE ${where.clause}
     LIMIT 1`,
    where.params
  );
  const targetTotal = periodRes.rows?.[0]?.target_total ?? 0;

  const dailyRes = await client.query(
    `SELECT target_date, target_value
     FROM ms_daily_targets
     WHERE ${where.clause}
     ORDER BY target_date ASC`,
    where.params
  );
  const dailyTargets = dailyRes.rows.reduce((acc, row) => {
    const key = row.target_date.toISOString().split("T")[0];
    acc[key] = Number(row.target_value || 0);
    return acc;
  }, {});
  ...
  return {
    ...
    departmentKey,
    metricKey,
    advisorUserId: Number.isFinite(advisorUserId) && advisorUserId > 0 ? advisorUserId : null,
    targetTotal: Number(targetTotal || 0),
    dailyTargets,
  };
}

async function upsertMsTargets(client, { scope, departmentKey, metricKey, periodId, advisorUserId, targetTotal, dailyTargets }) {
  const where = buildMsWhere({ scope, departmentKey, metricKey, periodId, advisorUserId });
  const updateRes = await client.query(
    `UPDATE ms_period_targets
     SET target_total=$${where.params.length + 1}, updated_at=now()
     WHERE ${where.clause}`,
    [...where.params, targetTotal]
  );

  if (!updateRes.rowCount) {
    await client.query(
      `INSERT INTO ms_period_targets
       (scope, department_key, metric_key, advisor_user_id, period_id, target_total)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [scope, departmentKey, metricKey, Number.isFinite(advisorUserId) ? advisorUserId : null, periodId, targetTotal]
    );
  }

  const entries = dailyTargets && typeof dailyTargets === "object" ? Object.entries(dailyTargets) : [];
  if (!entries.length) return;

  for (const [targetDate, targetValue] of entries) {
    const params = [...where.params, targetDate];
    const updateDaily = await client.query(
      `UPDATE ms_daily_targets
       SET target_value=$${params.length + 1}, updated_at=now()
       WHERE ${where.clause} AND target_date=$${params.length}`,
      [...params, targetValue]
    );
    if (!updateDaily.rowCount) {
      await client.query(
        `INSERT INTO ms_daily_targets
         (scope, department_key, metric_key, advisor_user_id, period_id, target_date, target_value)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [scope, departmentKey, metricKey, Number.isFinite(advisorUserId) ? advisorUserId : null, periodId, targetDate, targetValue]
      );
    }
  }
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (method === "GET" && path.endsWith("/ms-targets")) {
      const scope = qs.scope;
      const departmentKey = qs.departmentKey;
      const metricKey = qs.metricKey;
      const periodId = qs.periodId;
      const advisorUserId = qs.advisorUserId ? Number(qs.advisorUserId) : null;
      if (!scope || !departmentKey || !metricKey || !periodId) {
        return json({ error: "scope, departmentKey, metricKey, periodId are required" }, 400, headers);
      }
      if (scope === "personal" && !advisorUserId) {
        return json({ error: "advisorUserId required for personal scope" }, 400, headers);
      }

      const data = await getMsTargets(client, { scope, departmentKey, metricKey, periodId, advisorUserId });
      return json(data, 200, headers);
    }

    if (method === "PUT" && path.endsWith("/ms-targets")) {
      const { scope, departmentKey, metricKey, periodId, advisorUserId, targetTotal, dailyTargets } = body;
      if (!scope || !departmentKey || !metricKey || !periodId) {
        return json({ error: "scope, departmentKey, metricKey, periodId are required" }, 400, headers);
      }
      if (scope === "personal" && !advisorUserId) {
