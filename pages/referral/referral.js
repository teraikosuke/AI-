...
function normalizeCandidateStageKey(stage) {
  ...
  if (includesAny(['入社', '入社済', '入社予定', '採用', '就業', 'joined', 'hire', 'hired'])) return 'joined';
  if (includesAny(['内定', '内諾', 'オファー', 'offer'])) return 'offer';
  if (includesAny(['二次', '2次', 'second', 'interview2', '二次面接', '二次面談'])) return 'interview2';
  if (includesAny(['一次', '1次', 'first', 'interview1', '一次面接', '一次面談'])) return 'interview1';
  if (includesAny(['書類', 'document', 'doc', 'docscreen', 'doc_screen', 'document_screening'])) return 'docScreen';
  if (includesAny(['提案', '推薦', '紹介', '応募', 'エントリー', 'proposal', 'recommend', 'entry', 'apply'])) return 'proposal';
...
function buildFlowCandidatesFromSummaries(company) {
  ...
  const targetName = normalizeText(company?.company || company?.companyName || '');
...
function renderCompanyDetail() {
  ...
  const flow = stages.map((s, idx) => {
    ...
    const stageNames = stageCands.map(c => c.name).filter(name => name && name !== '-');
    const stageNameLines = stageNames.slice(0, 2);
    const stageNameExtra = Math.max(stageNames.length - stageNameLines.length, 0);
    const stageNameHtml = stageNameLines.length
      ? `<div class="hidden sm:block mt-0.5 text-[9px] sm:text-[10px] leading-tight text-white/90 max-w-[76px] text-center">
          ${stageNameLines.map(name => `<div class="truncate">${name}</div>`).join('')}
          ${stageNameExtra > 0 ? `<div class="opacity-80">他${stageNameExtra}名</div>` : ''}
        </div>`
      : '';
    const stageBadge = `
      <div class="flex flex-col items-center justify-center px-1">
        <div class="flex items-baseline gap-0.5">
          <span class="text-[11px] sm:text-base lg:text-lg font-semibold leading-tight">${s.value ?? 0}</span>
          <span class="text-[9px] sm:text-[10px]">件</span>
        </div>
        ${stageNameHtml}
      </div>
    `;
    ...
    return `
      ...
      ${idx < stages.length - 1 ? '<div class="flex items-center justify-center text-slate-300 w-5 sm:w-10 lg:w-14 flex-shrink-0" aria-hidden="true"><svg viewBox="0 0 24 24" class="w-4 h-4 sm:w-6 sm:h-6 lg:w-7 lg:h-7" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M13 6l6 6-6 6"/></svg></div>' : ''}
    `;
  }).join('');
